...
def test_check_empty_token(activity_token):
    ...
    activity_token.check_status_code_is_404()
    activity_token.check_text_is_url_not_found()
...
@allure.feature('Meme')
@pytest.mark.smoke
def test_delete_meme(delete_meme, auth_token, meme_id, get_mem_by_id):
...
@pytest.mark.parametrize('text, url, info', [
    ('1', '.com', {'rating': random.randint(1, 10), 'type': [5, 'avi'], 'user': faker.name()}),
    ('Name' * 10, "https://google.com'", {'rating': random.randint(-1, 0), 'type': [{}, 'JPEG'], 'user': '*/-_-\\'}),
])
